---
title: Seventy-six conjectures in mathematics
---

<p style="color:#999;font-size:0.8em;margin:0 0 1em 0;">Trial Project by Bryan Cheong</p>

# Seventy-six conjectures in mathematics
...
**63. Connected-pseudoforest binary rigidity (Conjecture 63).** The same one-field rigidity holds within the class of trees and connected unicyclic graphs. Connectedness is essential, since the implication fails for disconnected pseudoforests.
...
One tagging convention runs through every statement below: clauses marked [theorem] or “elementary” are proved; clauses marked “conditional proposition” follow from the hypotheses stated with them; the remaining clauses are conjectures; and open questions are labelled as such and are not counted among the seventy-six conjectures.
...
*Significance.* A single specialization cannot determine the polynomial in general, since the implication already fails inside the disconnected pseudoforests (see Conjecture 63), so the mechanism is that a hereditary class restriction removes the room in which the unweighted rank distribution and the size-weighted array can decouple. The nearest literature boundary is Rossmann's specialization theorem [77], which gives the hypothesis its meaning as the $$\mathbb F_2$$ census. The verification is a six-thousand-graph random holdout of bipartite graphs on nine to twelve vertices, spanning $$2747$$ distinct binary specializations, with no violation. A first decisive theorem would be the tree case, shared with Conjecture 63. The failure mode is a bipartite pair with equal binary specialization and distinct $$C$$, which would locate the boundary of one-field rigidity strictly inside the bipartite world and would redirect the class-restriction programme towards sparser families.

**Conjecture 63** *(Connected-pseudoforest binary rigidity)*. For $$G$$ and $$H$$ each a tree or a connected unicyclic graph, $$C_G(2,Y)=C_H(2,Y)$$ implies $$C_G=C_H$$.

*Remark.* Connectedness is essential: the implication fails for disconnected pseudoforests.

*Significance.* The mechanism is that on connected graphs with at most one cycle the binary rank distribution should already saturate the polynomial, and the remark pins where the mechanism stops, since dropping connectedness admits counterexamples among pseudoforests. The nearest literature boundary is the multiplicativity theorem of Rossmann [77], which is exactly the tool that builds the disconnected counterexamples and so marks the statement's edge. The verification is a six-thousand-graph random holdout of trees and connected unicyclic graphs on ten to thirteen vertices, spanning $$3192$$ distinct binary specializations, together with all unicyclic graphs on ten vertices, spanning $$656$$ binary classes. A first decisive theorem would be the tree case alone. The failure mode is a connected pair inside the class with equal binary specialization and distinct $$C$$, which would show that even one cycle leaves enough freedom to decouple the two arrays and would push the true rigidity class down to trees.

**Conjecture 64** *(Chordal binary rigidity)*. For chordal graphs $$G$$ and $$H$$, $$C_G(2,Y)=C_H(2,Y)$$ implies $$C_G=C_H$$.

*Significance.* Chordal graphs, those in which every cycle of length at least four has a chord, form the natural dense counterpart to the sparse classes of Conjectures 62 and 63, and the mechanism is the same class rigidity tested against a family with unbounded clique structure rather than bounded cycle structure. The nearest literature boundary is once more the specialization theorem [77] that interprets the hypothesis over $$\mathbb F_2$$. The verification is a six-thousand-graph random holdout of chordal graphs on nine to twelve vertices, spanning $$5598$$ distinct binary specializations, with no violation. A first decisive theorem would be the case of trees, which are chordal, followed by block graphs. The failure mode is a chordal pair separating the binary specialization from the polynomial, which would show that one-field rigidity is a sparsity phenomenon rather than a structural one, since it would then survive in Conjectures 62 and 63 while failing here.
...
### Conjecture 63: Connected-pseudoforest binary rigidity

A unicyclic graph is a connected graph with exactly one cycle, and a pseudoforest is a graph each of whose components has at most one cycle. The conjecture asserts the same one-field rigidity as Conjecture 62 within the class of trees and connected unicyclic graphs. Connectedness is essential, because the implication fails for disconnected pseudoforests, and Rossmann's product formula is exactly the tool that manufactures those counterexamples. The tests cover a six-thousand-graph random holdout on ten to thirteen vertices with $$3192$$ distinct binary specializations, together with all unicyclic graphs on ten vertices.
