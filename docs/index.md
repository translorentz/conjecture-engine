...
**87. Kirillov likelihood-ratio field monotonicity (Conjecture 87).** The normalized even-rank distribution of $$K_{P,q'}$$ is conjectured to dominate that of $$K_{P,q}$$ in monotone likelihood-ratio order for $$q<q'$$. It is tested only over $$q\in\{2,3,5\}$$.
...
**Conjecture 87** *(Kirillov likelihood-ratio field monotonicity)*. For every finite poset $$P$$ and prime powers $$q<q'$$, the normalized even-rank distribution defined by $$K_{P,q'}$$ dominates that defined by $$K_{P,q}$$ in monotone likelihood-ratio order.

*Remark.* The evidence tests only the field triples drawn from $$q=2$$, $$q=3$$, and $$q=5$$. The universal statement over every pair of prime powers is more tentative than the other conjectures of this part, the remaining cases being untested, and the likelihood-ratio order it asserts is precisely the strengthening that fails on the adjoint side in Conjecture 86.

*Significance.* Monotone likelihood-ratio order is strictly stronger than first-order dominance and compares the two distributions ratio by ratio across the even ranks. The conjecture asserts that on $$K$$ the coadjoint side satisfies this strengthening, which fails for $$A$$. The nearest related result is Delsarte [85], whose rank distributions of alternating forms are the model for the even-rank sequence. No decisive special case is proposed, and the tested range is the field range recorded in the remark. A counterexample would be a poset and a field pair reversing one likelihood ratio, which given the adjoint counterexample of Conjecture 86 is where the coadjoint monotonicity would be most likely to fail.
