...
**71. Interval support at fixed rank (Conjecture 71).** For every graph and every attainable rank $$r$$, the set $$K_r(G)=\{\vert S\vert :\rho_G(S)=r\}$$ is an interval of integers. The transposed statement is false, since the attainable ranks at a fixed support size need not form an interval.
...
**77. Adjoint–Kirillov determination (Conjecture 77).** Equality of the adjoint enumerators $$A_{P,q}$$ forces equality of the Kirillov enumerators $$K_{P,q}$$. The two enumerators are the two contractions of one bracket tensor, and the implication was verified across $$64$$ adjoint-collision classes.

**78. Lower-central recovery (Conjecture 78).** The adjoint enumerator determines the vector of lower-central factor dimensions $$(\dim\gamma_i(L_P)/\gamma_{i+1}(L_P))_i$$. This vector was constant on every adjoint-collision class in reverification.

**79. Ordinary Poincaré recovery (Conjecture 79, grand hypothesis).** The adjoint enumerator is conjectured to determine the ordinary cohomological Poincaré polynomial $$\sum_i\dim H^i(L_P,\mathbb F_q)U^i$$. It is a grand hypothesis whose tested core is only $$\dim H^2$$ and $$\dim H^3$$, resting on the deposited cohomology data rather than an independent recomputation.
...
**81. Adjoint Poincaré recovery (Conjecture 81, grand hypothesis).** The adjoint enumerator is conjectured to determine every adjoint-cohomology dimension $$\dim H^i(L_P,L_P)$$. It is a grand hypothesis whose tested core is only $$\dim H^1$$, $$\dim H^2$$, and $$\dim H^3$$ from the deposited data.
...
**85. Two-field arithmetic rigidity (Conjecture 85, grand hypothesis).** Equality of adjoint enumerators over $$\mathbb F_2$$ and $$\mathbb F_3$$ is conjectured to force equality over every finite field. It is a grand hypothesis whose only tested holdout beyond $$q=2,3$$ is $$q=5$$, on $$16$$ three-field collision classes.
...
**87. Kirillov likelihood-ratio field monotonicity (Conjecture 87, grand hypothesis).** The normalized even-rank distribution of $$K_{P,q'}$$ is conjectured to dominate that of $$K_{P,q}$$ in monotone likelihood-ratio order for $$q<q'$$. It is a grand hypothesis tested only over $$q\in\{2,3,5\}$$.
...
**90. Centre-one quotient shape (Conjecture 90).** When $$\dim Z(L_P)=1$$ the coefficient sequence of $$R_{P,q}$$ has interval support and is unimodal. Log-concavity failed four times, confirming unimodality as the boundary.
...
**Conjecture 71** *(Interval support at fixed rank)*. For every graph $$G$$ and every attainable rank $$r$$, the set $$K_r(G)=\{\vert S\vert :\rho_G(S)=r\}$$ is an interval of integers.

*Significance.* This is the support statement beneath the positivity statements: strict log-concavity of a slice presupposes gapless support, and the conjecture asserts gaplessness for every graph, not only connected ones. The mechanism would be an exchange argument, passing between witnesses of the same rank whose sizes differ by one, and the direction of the statement is essential, since the transposed assertion is false: the attainable ranks at a fixed support size need not form an interval. The interval structure, if real, is therefore a property of the size direction alone. The nearest literature boundary is the support analysis implicit in Rossmann's connected-dominating-set theorem [77], which reads off one extreme stratum of the array. The verification is the full seven-vertex atlas and twenty-five hundred random graphs on eight to ten vertices. A first decisive theorem would be the interval property for trees. The failure mode is a graph with a gapped rank level set, which would break the fibrewise programme of Conjecture 70 at the level of supports and, given the falsity of the transposed statement, would show that neither direction of the array has interval structure.
...
*Significance.* The diametral chain of the lemma realizes $$\operatorname{diam}(T)+1$$ values, and equality requires the whole subset lattice to produce nothing beyond the chain, so the mechanism parallels Conjecture 75 with the diameter replacing the internal vertex count, and the two rigidity statements pin the same extremal pair from two independent directions, each serving as a cross-check on the other. The nearest literature boundary is again the lemma itself. The verification is exhaustive over all trees through fourteen vertices. A first decisive theorem would be the forward half for the path, where the diameter is $$n-1$$ and the claimed equality reads $$s(P_n)=n$$. The failure mode is a tree of small diameter and few rank values outside the pair, which would show the diameter bound to be loose in a structured way and would decouple the two rigidity statements that the exhaustive search through fourteen vertices keeps locked together.

## Part V: pattern Lie algebras of finite posets

*Added to the deposit: 6 August 2026.*

The twenty conjectures below concern the pattern Lie algebras of finite posets, approached through two rank-weight enumerators that record how the bracket contracts against an element and against a functional. They form four programmes. A determination programme (Conjectures 77 to 82) asserts that the adjoint enumerator alone already fixes the coadjoint enumerator and a list of classical invariants. A characteristic-and-field programme (Conjectures 83 to 87) locates what in these distributions is independent of the ground field and how the field moves them when it does. A centre-one programme (Conjectures 88 to 90) studies the incidence quotient that measures the gap between the two enumerators when the centre is a line. A Hasse-forest and unitriangular programme (Conjectures 91 to 96) reads coadjoint structure off the shape of the cover graph and pins the unitriangular chain as the extremal case. The proved layer is separated from the conjectural one: the incidence identity is a theorem with a full proof, the standard adjoint–coadjoint duality of this family, included for calibration and not claimed as new, and every conjecture is stated against that floor. Part V contains sixteen principal conjectures and four grand hypotheses. The grand hypotheses, Conjectures 79, 81, 85, and 87, are retained for provenance and marked in their titles, each carrying in a remark the narrow tested core that its evidence actually supports rather than the universal claim its title names.
...
*Significance.* The adjoint and Kirillov enumerators are the two contractions of one bracket tensor, one against elements and one against functionals, and the proposition already forces them to agree at $$T=1$$ and $$T=1/q$$, so the mechanism is that the adjoint distribution should carry enough of the tensor to reconstruct its transpose across the whole polynomial and not only at those two points. The nearest literature boundary is Delsarte's [85] analysis of the rank distributions of alternating bilinear forms over a finite field, which governs the Kirillov side in isolation but supplies no bridge from the adjoint side. A first decisive theorem is the chain case $$L=\mathrm{ut}_n(\mathbb F_q)$$, where the divisibility of the proposition already ties the two enumerators together. In our reverification the adjoint enumerator determined the Kirillov enumerator across $$64$$ adjoint-collision classes with no failure. The failure mode is an adjoint-collision pair with distinct Kirillov enumerators, which would show that the coadjoint contraction retains information the adjoint contraction discards.
...
*Significance.* The rank of $$\operatorname{ad}_x$$ is the dimension of the image $$[x,L_P]$$, and the lower central series is assembled from exactly these images as $$x$$ ranges over the algebra, so the mechanism is that the distribution of adjoint ranks aggregates enough of the iterated-bracket data to fix the graded dimensions $$\dim\gamma_i/\gamma_{i+1}$$. The nearest literature boundary is the breadth theory of Cameron–Coll–Mayers–Russoniello [80], in which the top adjoint rank $$b_A$$ is a single coordinate of the object the whole enumerator refines. A first decisive theorem is the chain, where $$\mathrm{ut}_n$$ has a known lower central series matched to its adjoint spectrum. In our reverification the lower-central factor vector was constant on every adjoint-collision class. The failure mode is an adjoint-collision class carrying two distinct factor vectors, which would make the nilpotency filtration invisible to the adjoint distribution.

**Conjecture 79** *(Ordinary Poincaré recovery, grand hypothesis)*. For every finite poset $$P$$ and every prime power $$q$$, the enumerator $$A_{P,q}(T)$$ determines the ordinary cohomological Poincaré polynomial $$\sum_i\dim H^i(L_P,\mathbb F_q)U^i$$.

*Remark.* This is a grand hypothesis. The evidence tests only the equality of $$\dim H^2(L_P,\mathbb F_q)$$ and $$\dim H^3(L_P,\mathbb F_q)$$ across adjoint-collision classes, and it rests on the deposited cohomology computation rather than on an independent recomputation of Lie-algebra cohomology. The universal statement over all degrees is retained for provenance and is not backed by evidence comparable to the principal list.

*Significance.* The Chevalley–Eilenberg differential of $$L_P$$ is assembled from the same structure constants that the adjoint maps expose, so the mechanism is that the adjoint-rank distribution should constrain the cohomological Betti numbers through the ranks of those differentials. The nearest literature boundary is the poset Lie cohomology of Lampret–Vavpetič [82], the closest computation of $$H^i(L_P,\mathbb F_q)$$ for this family. As a grand hypothesis it offers no decisive theorem, and its tested core is the two degrees recorded in the remark. The failure mode is an adjoint-collision class with distinct ordinary Betti numbers in some degree, which would place cohomology beyond the reach of the adjoint enumerator.
...
*Significance.* A derivation is constrained at every point by its interaction with the adjoint action, so the derivation algebra is a functional of the adjoint representation whose coarse image is the adjoint-rank distribution, and the mechanism is that this distribution should fix the dimension of the space of derivations. The nearest literature boundary is again the breadth theory of Cameron–Coll–Mayers–Russoniello [80], which reads structural invariants of $$L_P$$ off the same adjoint data. A first decisive theorem is the chain, where $$\dim\operatorname{Der}(\mathrm{ut}_n)$$ is classical. In our reverification $$\dim\operatorname{Der}(L_P)$$ was constant on every adjoint-collision class. The failure mode is an adjoint-collision class with distinct derivation dimensions, which would show that the derivation algebra sees structure the adjoint distribution cannot.

**Conjecture 81** *(Adjoint Poincaré recovery, grand hypothesis)*. For every finite poset $$P$$ and every prime power $$q$$, the enumerator $$A_{P,q}(T)$$ determines the adjoint-cohomology dimensions $$\dim H^i(L_P,L_P)$$ for every $$i$$.

*Remark.* This is a grand hypothesis. The evidence tests only the equality of $$\dim H^1(L_P,L_P)$$, $$\dim H^2(L_P,L_P)$$, and $$\dim H^3(L_P,L_P)$$ across adjoint-collision classes, and it rests on the deposited cohomology computation rather than on an independent recomputation. The universal statement over all degrees is retained for provenance and is not backed by evidence comparable to the principal list.

*Significance.* Cohomology with coefficients in the adjoint module governs the deformation theory of $$L_P$$, and its differentials are built from the same brackets whose ranks the adjoint maps record, so the mechanism parallels Conjecture 79 with the algebra acting on itself. The nearest literature boundary is the poset Lie cohomology of Lampret–Vavpetič [82]. As a grand hypothesis it offers no decisive theorem, and its tested core is the three degrees recorded in the remark. The failure mode is an adjoint-collision class with distinct adjoint Betti numbers in some degree, which would separate the deformation theory from the adjoint enumerator.
...
*Significance.* The centroid is by definition the space of linear maps commuting with every $$\operatorname{ad}_x$$, so it is a direct invariant of the adjoint representation whose coarsest shadow is the adjoint-rank distribution, and the mechanism is that this distribution should already pin its dimension. The nearest literature boundary is once more the breadth theory of Cameron–Coll–Mayers–Russoniello [80], whose invariants live on the same adjoint data. A first decisive theorem is the chain, where the centroid of $$\mathrm{ut}_n$$ is a scalar line. In our reverification $$\dim\operatorname{Cent}(L_P)$$ was constant on every adjoint-collision class. The failure mode is an adjoint-collision class with distinct centroid dimensions, which would show that the commutant of the adjoint action is not determined by its rank statistics.
...
*Significance.* The attained adjoint ranks are governed by which support patterns of $$x$$ can force a given image dimension, a combinatorial condition on $$P$$, so the mechanism is that the set of attained values is fixed by the poset while only the multiplicities move with the field. The nearest literature boundary is Halasi–Pálfy [83], whose theorem that pattern-group class counts are not polynomial in $$q$$ shows that the counting side genuinely depends on the field, against which the invariance of the mere support is the sharper claim. A first decisive theorem is the chain, where the attained ranks of $$\mathrm{ut}_n$$ are an interval fixed by $$n$$. In our reverification the attained-rank sets agreed across $$\mathbb F_2$$, $$\mathbb F_3$$, and $$\mathbb F_5$$ in every case. The failure mode is a poset with an adjoint rank attained over one field but not another, which would make even the support of the distribution characteristic-dependent.
...
*Significance.* The attained Kirillov ranks are the even numbers realized as ranks of the alternating forms $$B_f$$, and which of these occur is a combinatorial condition on the coadjoint action of $$P$$, so the mechanism mirrors Conjecture 83 on the coadjoint side. The nearest literature boundary pairs two sources: Delsarte [85] for the rank distributions of alternating forms and Halasi–Pálfy [83] for the field dependence of the associated class counts, against which the invariance of the support is the finer statement. A first decisive theorem is the chain. In our reverification the attained Kirillov-rank sets agreed across $$\mathbb F_2$$, $$\mathbb F_3$$, and $$\mathbb F_5$$ in every case. The failure mode is a poset with a Kirillov rank attained over one field but not another, which would locate genuine field dependence already in the coadjoint support.

**Conjecture 85** *(Two-field arithmetic rigidity, grand hypothesis)*. For all finite posets $$P$$ and $$Q$$, if $$A_{P,2}=A_{Q,2}$$ and $$A_{P,3}=A_{Q,3}$$ then $$A_{P,q}=A_{Q,q}$$ for every prime power $$q$$.

*Remark.* This is a grand hypothesis. Among the posets computed over all three fields, the only field beyond $$q=2$$ and $$q=3$$ at which equality was actually tested is the single holdout $$q=5$$. The universal statement over every prime power is retained for provenance and is not backed by evidence comparable to the principal list.

*Significance.* Two adjoint enumerators over the two smallest fields are two weighted shadows of the poset's rank array, and the mechanism is rigidity of the arrays that posets realize, so that no third field can carry information absent from the first two. The nearest literature boundary is Halasi–Pálfy [83], whose non-polynomiality theorem is exactly the warning that larger fields can behave differently, against which the two-field claim asserts that this family is nonetheless pinned. As a grand hypothesis it offers no decisive theorem, and its tested core is the single holdout recorded in the remark: equality of $$A$$ at $$q=2$$ and $$q=3$$ forced equality at $$q=5$$ across $$16$$ three-field collision classes. The failure mode is a pair agreeing over $$\mathbb F_2$$ and $$\mathbb F_3$$ yet separated over some larger field.
...
*Significance.* First-order stochastic dominance compares two distributions through all of their tail probabilities at once, so the conjecture asserts that raising the field can only push the adjoint rank upward in the strong tailwise sense, the mechanism being that a larger field offers more elements of high rank at every support pattern. The nearest literature boundary is the breadth theory of Cameron–Coll–Mayers–Russoniello [80], in which the top rank $$b_A$$ is stable while the bulk of the distribution shifts. The statement is calibrated to the strongest surviving form: the monotone-likelihood-ratio strengthening is false, failing three times already from $$q=2$$ to $$q=3$$, so first-order dominance is the honest boundary. In our reverification first-order dominance held in all $$221$$ field-pair tests, while the likelihood-ratio strengthening failed as recorded. The failure mode is a poset and a field pair reversing one tail probability, which would show that even first-order dominance is too strong.

**Conjecture 87** *(Kirillov likelihood-ratio field monotonicity, grand hypothesis)*. For every finite poset $$P$$ and prime powers $$q<q'$$, the normalized even-rank distribution defined by $$K_{P,q'}$$ dominates that defined by $$K_{P,q}$$ in monotone likelihood-ratio order.

*Remark.* This is a grand hypothesis. The evidence tests only the field triples drawn from $$q=2$$, $$q=3$$, and $$q=5$$. The universal statement over every pair of prime powers is retained for provenance and is not backed by evidence comparable to the principal list, and the likelihood-ratio order it asserts is precisely the strengthening that fails on the adjoint side in Conjecture 86.

*Significance.* Monotone likelihood-ratio order is strictly stronger than first-order dominance, ordering the two distributions ratio by ratio across the even ranks, so the conjecture asserts that the coadjoint side is rigid enough to support on $$K$$ the very strengthening that fails for $$A$$. The nearest literature boundary is Delsarte [85], whose rank distributions of alternating forms are the ambient model for the even-rank sequence. As a grand hypothesis it offers no decisive theorem, and its tested core is the field range recorded in the remark. The failure mode is a poset and a field pair reversing one likelihood ratio, which given the adjoint counterexample of Conjecture 86 would be the expected place for the coadjoint monotonicity to break as well.
...
*Significance.* Conjecture 77 runs the duality from the adjoint side to the coadjoint side, and this is the reverse implication under the hypothesis that the centre is a line, the mechanism being that a one-dimensional centre leaves the incidence quotient of the proposition rigid enough to be inverted. The nearest literature boundary is Delsarte [85], whose alternating-form rank distributions are what $$K_{P,q}$$ records. A first decisive theorem is the chain $$\mathrm{ut}_n$$, whose centre is the single corner entry. In our reverification the Kirillov enumerator determined the adjoint enumerator on all $$43$$ centre-one records. The failure mode is a centre-one pair with equal $$K$$ and distinct $$A$$, which would show that even a line centre leaves the coadjoint side blind to part of the adjoint distribution.
...
*Significance.* The quotient $$R_{P,q}$$ measures the gap between the coadjoint and adjoint enumerators after the two common roots of the proposition are divided out, and the mechanism is that under a line centre this residual difference is a genuine counting polynomial rather than a signed one, so its coefficients are nonnegative integers. The nearest literature boundary is Delsarte [85], against whose alternating-form model the positivity of $$R_{P,q}$$ is a structural constraint on how far $$K$$ can exceed $$A$$. A first decisive theorem is the chain, where $$R$$ is computable in closed form. In our reverification $$R_{P,q}$$ had nonnegative integer coefficients on all $$43$$ centre-one records. The failure mode is a centre-one poset with a negative coefficient in $$R_{P,q}$$, which would show that the incidence quotient is not a counting object and would remove the ground for the shape statement of Conjecture 90.

**Conjecture 90** *(Centre-one quotient shape)*. For every finite poset $$P$$ with $$\dim Z(L_P)=1$$ and every prime power $$q$$, the coefficient sequence of $$R_{P,q}(T)$$ has interval support and is unimodal.

*Significance.* Given the positivity of Conjecture 89, the natural next question is the shape of the coefficient sequence, and the mechanism is that the incidence quotient should behave like a well-formed rank polynomial with gapless support and a single peak. The nearest literature boundary pairs Delsarte [85] for the alternating-form arithmetic with Coll–Mayers [81] for the coadjoint-orbit reading of $$K$$. The statement is calibrated to the strongest form the data support: log-concavity is false, so unimodality is the honest boundary. In our reverification $$R_{P,q}$$ had interval support and was unimodal throughout, and log-concavity failed four times, confirming unimodality as the boundary. The failure mode is a centre-one poset whose $$R_{P,q}$$ has a gap or a second peak, which would isolate the positivity of Conjecture 89 as the whole of its regularity.
...
*Significance.* This is the reverse determination of Conjecture 77 under a structural hypothesis complementary to the centre-one hypothesis of Conjecture 88, the mechanism being that a forest cover graph forces the coadjoint form to be built from independent covers, leaving the incidence quotient rigid enough to invert. The nearest literature boundary is Coll–Mayers [81], whose index computations for Lie poset algebras are sharpest on tree-shaped posets. A first decisive theorem is the chain, a forest on one branch. In our reverification the Kirillov enumerator determined the adjoint enumerator across $$44$$ forest-collision classes. The failure mode is a forest pair with equal $$K$$ and distinct $$A$$, which would show that the forest hypothesis is not the right structural home for reverse determination.
...
*Significance.* An alternating form has even rank, and the conjecture asserts that on a forest every admissible even rank up to the top is realized by some functional, the mechanism being that the independent covers of a forest let the coadjoint rank be tuned one pair at a time. The nearest literature boundary pairs Coll–Mayers [81], whose index is the codimension complementary to $$b_K$$, with Delsarte [85], whose alternating-form model predicts which even ranks can occur. A first decisive theorem is the chain, where $$K_{\mathrm{ut}_n}$$ saturates its even range. In our reverification the forest Kirillov support filled every even rank up to $$b_K$$ in each case. The failure mode is a forest with a gap in its Kirillov support, which would break the support hypothesis beneath Conjecture 93.
...
*Significance.* On the gapless support of Conjecture 92 the next question is the shape of the even-rank counts, and the mechanism is that the coadjoint rank distribution of a forest should rise to a single peak and fall, with no interior dip. The nearest literature boundary pairs Delsarte [85] for the alternating-form counts with Coll–Mayers [81] for the coadjoint reading. The statement is calibrated to the strongest surviving form: strict log-concavity is false, so unimodality is the honest boundary. In our reverification the forest even-rank sequence was unimodal throughout. The failure mode is a forest whose even-rank sequence has two peaks, which would confine positivity of shape to the unitriangular chain of Conjecture 95.
...
*Significance.* The bound ties the maximal coadjoint-orbit rank to the adjoint breadth, and the mechanism is that on a forest the largest alternating form cannot outrun twice the largest adjoint image, since both are driven by the same covers. The nearest literature boundary pairs Cameron–Coll–Mayers–Russoniello [80] for the breadth $$b_A$$ with Coll–Mayers [81] for the index that controls $$b_K$$. The statement is calibrated by class: the unrestricted inequality is false, with many counterexamples off the forest class, so the forest hypothesis is doing the work. In our reverification the forest bound held everywhere, and the unrestricted bound failed fourteen times off the forest class. The failure mode is a forest violating $$b_K\le 2b_A$$, which would show that even tree-shaped posets can carry a coadjoint orbit disproportionate to their adjoint breadth.
...
*Significance.* The chain is the extremal forest, and here the mere unimodality of Conjecture 93 sharpens to strict log-concavity, the mechanism being that the coadjoint rank distribution of $$\mathrm{ut}_n$$ is as concentrated as the family allows. The nearest literature boundary is Marberg [84], whose character and orbit enumeration for the unitriangular group is the closest account of the coadjoint data of $$\mathrm{ut}_n$$. A first decisive theorem would be strict log-concavity for a single infinite family of indices, say the central ones. In our reverification strict log-concavity was confirmed for $$\mathrm{ut}_4$$, $$\mathrm{ut}_5$$, and $$\mathrm{ut}_6$$ over $$\mathbb F_2$$, for $$\mathrm{ut}_4$$ and $$\mathrm{ut}_5$$ over $$\mathbb F_3$$, and for $$\mathrm{ut}_4$$ over $$\mathbb F_5$$. The failure mode is an interior index of some $$\mathrm{ut}_n$$ where the strict inequality fails, which would show that even the most structured chain stops short of strict concentration.
...
*Significance.* The incidence variety of the proposition couples an element to a functional annihilating its adjoint image, and the mechanism is that a high adjoint rank leaves little room for the coadjoint form to be large as well, so the two ranks pull against each other on $$I_L$$. The nearest literature boundary is Coll–Mayers [81], whose coadjoint-orbit analysis of Lie poset algebras is the frame in which $$\operatorname{rank}(B_f)$$ is an orbit dimension. A first decisive theorem is the abelian case, where both ranks are identically zero and the covariance vanishes, giving the equality clause. In our reverification the covariance was negative on every nonabelian poset tested and exactly zero on all sixteen abelian ones. The statement is calibrated to the honest strength: the stronger conditional stochastic-order and conditional-mean forms are false, so the sign of a single covariance is the surviving claim, and the failure mode is a nonabelian poset with nonnegative incidence covariance, which would decouple the two ranks on the very variety that defines their duality.
...
### Conjecture 71: Interval support at fixed rank

Beneath any log-concavity claim lies a support claim, since a sequence with internal gaps cannot be strictly log-concave. The conjecture asserts that for every graph and every attainable rank $$r$$, the set $$K_r(G)$$ of sizes of vertex subsets achieving rank $$r$$ is an interval of integers, with no gaps. The transposed statement is false, because the attainable ranks at a fixed subset size need not form an interval, so the asserted structure is a genuine asymmetry between the size direction and the rank direction of the array. The expected mechanism is an exchange argument moving between same-rank witnesses whose sizes differ by one.
...
The Lie-algebra cohomology $$H^i(L_P,\mathbb F_q)$$ is computed from the Chevalley–Eilenberg complex, whose differentials are assembled from the structure constants of the bracket, and its dimensions are packaged in the Poincaré polynomial $$\sum_i\dim H^i U^i$$. The conjecture asserts that the adjoint enumerator, which sees those same structure constants only through the ranks of the adjoint maps, nonetheless determines the whole Poincaré polynomial. This is a grand hypothesis: the tested evidence reaches only the second and third cohomology dimensions and rests on the deposited cohomology data rather than an independent recomputation. The universal claim over all degrees is retained for provenance and flagged as such.
...
Cohomology with coefficients in the adjoint module, $$H^i(L_P,L_P)$$, is the invariant that controls deformations of the Lie algebra, with $$H^2$$ classifying infinitesimal deformations and $$H^1$$ the outer derivations. The conjecture asserts that the adjoint enumerator determines all of these dimensions. Like Conjecture 79 it is a grand hypothesis: the tested core reaches only the first three adjoint-cohomology dimensions and rests on the deposited data, while the universal statement over all degrees is kept for provenance and marked as not comparably supported.
...
Evaluating over the two smallest fields gives two weighted shadows of a poset's rank array. The conjecture asserts that these already determine the array over every finite field: if two posets have equal adjoint enumerators over $$\mathbb F_2$$ and over $$\mathbb F_3$$, they agree over all $$\mathbb F_q$$. This is a grand hypothesis, and its only tested holdout beyond $$q=2,3$$ is the single field $$q=5$$, on $$16$$ three-field collision classes. Since Halasi and Pálfy show larger fields can behave differently in general, the claim is that this particular family is nonetheless rigid, and it is retained for provenance with that narrow core stated plainly.
...
### Conjecture 87: Kirillov likelihood-ratio field monotonicity

Monotone likelihood-ratio order is the strong comparison under which the ratio of the two probability mass functions is monotone across the ranks, and it implies but is not implied by first-order dominance. The conjecture asserts that the coadjoint even-rank distributions are rigid enough to satisfy this strong ordering as the field grows, precisely the strengthening that fails for the adjoint side in Conjecture 86. It is a grand hypothesis, tested only over the fields $$q\in\{2,3,5\}$$, and retained for provenance with that core stated.
...
### Conjecture 90: Centre-one quotient shape

A finite integer sequence is unimodal if it rises to a single peak and then falls, and it has interval support if its nonzero entries occupy a gapless block, while log-concavity, the condition $$a_i^2\ge a_{i-1}a_{i+1}$$, is strictly stronger than unimodality for positive sequences. The conjecture asserts that the coefficient sequence of the incidence quotient $$R_{P,q}$$ is unimodal with interval support whenever the centre is a line. The calibration is exact: log-concavity is false, failing four times, so unimodality is the boundary the data support.
