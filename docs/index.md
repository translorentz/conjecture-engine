...
the aggregated multigraded Betti numbers of the Stanley–Reisner ring by Hochster's formula [90], graded by restriction size and nullity, with strand polynomials as in Part VII.
...
Reading a nullity strand as a polynomial and stripping its leading monomial, every zero is conjectured to lie strictly in the left half of the complex plane. Real-rootedness fails — the strand (3,2,1) has nonreal roots — but those roots still obey the stability law, exactly as for the graph strands of Part VII, whose statement this extends from graphs to all simple binary matroids. All 1,324 strands passed exact rational Routh tests.
