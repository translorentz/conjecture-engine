...
*Significance.* The Chevalley–Eilenberg differential of $$L_P$$ is built from the same structure constants that the adjoint maps record, so the adjoint-rank distribution constrains the cohomological Betti numbers through the ranks of those differentials. The nearest related result is the poset Lie cohomology of Lampret–Vavpetič [82], the closest computation of $$H^i(L_P,\mathbb F_q)$$ for this family. As a grand hypothesis it comes with no decisive theorem, and its tested core is the two degrees recorded in the remark. A counterexample would be an adjoint-collision class with distinct ordinary Betti numbers in some degree, which would show that cohomology is not determined by the adjoint enumerator.
...
*Significance.* Cohomology with coefficients in the adjoint module governs the deformation theory of $$L_P$$, and its differentials are built from the same brackets whose ranks the adjoint maps record, so the situation parallels Conjecture 79 with the algebra acting on itself. The nearest related result is the poset Lie cohomology of Lampret–Vavpetič [82]. As a grand hypothesis it comes with no decisive theorem, and its tested core is the three degrees recorded in the remark. A counterexample would be an adjoint-collision class with distinct adjoint Betti numbers in some degree, which would show that the deformation theory is not determined by the adjoint enumerator.
...
*Significance.* Monotone likelihood-ratio order is strictly stronger than first-order dominance and compares the two distributions ratio by ratio across the even ranks. The conjecture asserts that on $$K$$ the coadjoint side satisfies this strengthening, which fails for $$A$$. The nearest related result is Delsarte [85], whose rank distributions of alternating forms are the model for the even-rank sequence. As a grand hypothesis it comes with no decisive theorem, and its tested core is the field range recorded in the remark. A counterexample would be a poset and a field pair reversing one likelihood ratio, which given the adjoint counterexample of Conjecture 86 is where the coadjoint monotonicity would be most likely to fail.
...
*Significance.* Given the positivity of Conjecture 89, the next question is the shape of the coefficient sequence. The conjecture asserts that $$R_{P,q}$$ has gapless support and a single peak. The nearest related results are Delsarte [85] for the alternating-form arithmetic and Coll–Mayers [81] for the coadjoint-orbit reading of $$K$$. The statement is calibrated to the strongest form the data support. Log-concavity is false, so unimodality is the appropriate form. In tests on the corpus described in the stress-test section, $$R_{P,q}$$ had interval support and was unimodal throughout, and log-concavity failed four times. A counterexample would be a centre-one poset whose $$R_{P,q}$$ has a gap or a second peak, which would show that the positivity of Conjecture 89 is its only regularity.
