...
| C10 | **Factorial twins**: n=3 is the only n with n!−1, n!+1 both prime (convergent Borel–Cantelli); primorial analogue attributed to Lillie after priority failure (Finding f:c10) | [research] | `c10_factorial_twins.py` |
...
| C16 | Uniform quantitative de Polignac: π_d ~ S(d)Li₂ uniformly, d ≤ (log x)^A; **(ii) residual field Gaussian with covariance kernel derived from HL triple constants** (mean ρ = 0.43 at 1e8; diagonal 4-tuple deficit stated open) | [bench] (i); [research] (ii) | `c16_uniform_depolignac.py`, `c16b_covariance_kernel.py` |
...
| C19 | First-occurrence gaps: liminf over realized gaps of log p(g)/√g = √(e^γ/2) = 0.943682 (registered against slope-1 laws) | [research] | `c19_gap_first_occurrence.py` |
...
| C21 | **Twin race mod 5/8**: Lemma (orientation elimination) + drift law M_x((D₁−T)/√π_t)→0, null covariance, conditional sign density ½, mod-8 companion D₇ ~ 2T | [research] | `c21_twin_race_mod5.py`, `c21b_twin_race_mod8.py` |
| C22 | Least primes: (i) Exp(1)/Gumbel; **(ii) deficit θ_disc (defined Cramér–Bernoulli control) + θ_corr > 0 persisting on prime moduli** (stratified experiment differentiates from Leung's smooth-q effects) | [bench] (i); [research] (ii) | `c22_least_prime_ap.py`, `c22b_stratified.py` |
