...
exact constant catalogued as an OEIS entry (A188596), and others
...
and re-verified at our bounds, because the calibration layer is what
makes the novel layer's constants credible.  Second, every
falsifiable-by-instance statement was verified against exact counts and
then re-tested well past its original bound, with the search treated as
an attempt at refutation rather than confirmation; one such search was
decisive against a candidate statement before publication, and appears
as the calibration clause of Conjecture~\ref{c9}.  Third, constants and
...
\ref{c12} and \ref{c20}).  Every statement was novelty-searched at neighbourhood
...
(both $T/\sqrt\pi$ and its multiples have logarithmic mean $\to0$).  Convergence of $\M_x$ at
...
whose constants are derived, not fitted.  One question the family raises is recorded as
...
$\omega(3)=2$.  Unlike an arbitrary shifted pair, the companion here is structurally
...
OEIS A188596; verified at $z=-1.06$ at $10^7$): infinitely many prime bases have prime
...
probability, not number theory: granting the invariance principle, $D(t)\approx B(N(t))$
...
\emph{size} of the correction: an underestimated constant, genuine cross-dependence,
...
($z=-1.6$, a deficit confirmed by independent recomputation); $29$ Lucas prime indices $\le10^4$; the
...
which is why the clause is restricted to $n\ge4$.  Hence among all offsets of bounded size the only
...
derivation, not just an assertion: a generic twin pair near $n!$ would carry the Hardy--Littlewood twin
...
quantities $\kappa_S$, which make no independence assumption.  The computation then returned a small
...
checks consistency, not the constant.  The OEIS records further candidate indices beyond our completed
range:
...
caveat that data to $H=3000$ cannot distinguish divergence from a pure $\log$ with a large constant
...
claim through $|F(m)-F(j)|$).  Every representation problem $F(m)=p+F(j)$ in this range
...
between-candidate dependence that the pair term carries.  (The $(3,3)$-lane refined
...
\cite{Martin}---is included as an attributed calibration benchmark: exhaustively
...
deterministic cumulative counts are otherwise undefined: for $t$ uniform on $[x,2x]$ and a window length $H$, let
...
are handled inside the kernels, not ignored: whenever the four-point set collapses (repeated
...
registered identity-hunting programme.  Verified on $150$ log-sampled $n\le10^8$: the profile
...
A007534 complete---verified to $10^8$, re-swept to $10^9$, independently re-verified by a different algorithm, same
...
not the extreme-value framework.  One selection caveat: sampling uniformly over the \emph{realized}
gaps in
...
while $\mu$ is left as the one unresolved constant of the clause.  (The restriction to $\mathcal G$ is not cosmetic: $p(g)$ is not known
...
(the scale is not $1/\log x$).  In contrast to Chebyshev's races, the twin race has no
persistent leader;
...
yields were derived first and tested second.  At $x=10^9$ the predicted drift is far inside the
...
one noise unit of zero in log-mean.  This is moderately strong directional evidence---two independent
...
$\M_x$-clauses above say exactly what is conjectured and no more.  Two model assumptions
...
mechanism and (as a mod-$3$ computation shows, overturning the natural first guess
...
model-free $\Theta(q)$ alone.  The named benchmark (the parity-aware Cram\'er--Bernoulli
...
$1/\log q$ decay that the flat measured values do not show.  (One might expect the effect
to sit at the full deficit scale, since the \emph{affected-class
fraction} is $\pi(q)/\varphi\asymp1/\log q$; but the deficit is the affected fraction times the collision
...
stated programme, runs as follows.  Let $V_q(y)$ be the number of reduced classes mod $q$ not
...
and no less than the model earns: the upper bound alone would be consistent with
...
statement whose additive constant is uncalibrated, not a forecast.  Consistency data: $\{1093,3511\}$ below $10^8$ against
...
open component.  (The order of this sum matters, and is easy to get wrong by a factor
...
strongly with $n$---most drastically at $p=3$: for $n\equiv1\pmod3$, $3\mid n-q^2$ for \emph{every} prime
