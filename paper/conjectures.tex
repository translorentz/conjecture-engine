...
there is $\eta_B>0$ with
\[
\sup_{\substack{2\le d\le(\log N)^{B}\\ 2\mid d}}
\Bigl|\frac{\pi^{\mathrm q}_d(N)}{C(d)\,I_d(N)}-1\Bigr|
\;\ll_B\;(\log N)^{-\eta_B}:
\]
the profile is matched at a quantified uniform rate, not merely
$d$-by-$d$.  In particular ($d=2$, the classical instance
...
family, not cherry-picked at one shift.  Two questions the family
raises, recorded as open rather than conjectured: the maximal range
$D(N)$ for which the uniform law can hold (the analogue of the
Elliott--Halberstam question for this family---whether there is a
sharp transition at some power scale $N^{\delta}$ rather than a
logarithmic one), and the distributional behaviour of the constants
$C(d)$ themselves (mean, extremes, and dependence on the
factorization of $d$) as $d$ ranges over an interval.
...
\emph{(ii)} [occupation law] under the same centered Gaussian
fluctuation model as Conjecture~\ref{c21}(iii), the logarithmic
occupation measure of leadership,
$\frac1{\log x}\int_2^x\mathbf 1_{\{D(t)>0\}}\,\frac{dt}t$, does not
converge to a constant but has the arcsine-type limiting distribution
of the corresponding random walk---long unbalanced excursions by
either class are the \emph{prediction}, and a persistent
systematic leader is the refutation.
\end{conjecture}
...
\begin{conjecture}[Factorial twins; explicit prior statement not
found]\label{c10}
\emph{(i)} [window rigidity, elementary] for $2\le|a|\le n$ the
number $n!+a$ is composite (any prime factor of $a$ is at most $n$
and divides both terms), so among all offsets of bounded size the
only candidates for primality near $n!$ are $a=\pm1$: the twin
question below is the \emph{unique} bounded-offset constellation
question at $n!$, and every admissible offset set is a subset of
$\{-1,+1\}$.
\emph{(ii)} [uniqueness] Only finitely many $n$ have $n!-1$ and
$n!+1$ simultaneously prime, and the complete list is $n=3$: i.e.\
$6=3!=3\#$ is the unique factorial lying between twin primes.
\emph{(iii)} [joint independence] the two single-sided counts
$F_{\pm}(N)=\#\{n\le N:\ n!\pm1 \text{ prime}\}$ are each
$\sim\e^{\gamma}\log N$ and asymptotically independent: their
difference $F_{+}-F_{-}$, normalized by
$\sqrt{2\e^{\gamma}\log N}$, is asymptotically standard normal.
\end{conjecture}
...
infinitude.  The independence in (iii) is not automatic but has a
reason the model can state: the shared screening (coprimality to all
$p\le n$) is \emph{deterministic}---both neighbors always pass
it---so no correlation is induced by it, unlike generic twin
constellations whose joint singular series couples the events.
...
than the tail estimate can guarantee, and chosen deliberately as the
most falsifiable statement.  On priority we are careful twice over
(Finding~\ref{f:c10}): the two single-sided lists are classical, the
coincidence at $n=3$ is visible to anyone who reads them, and the
fifth external review asserts the uniqueness statement has been
recorded before, though without a citation; our abstract-depth search
found no explicit prior statement, and we label the slot accordingly
---the claim is at most the Goldilocks-maximal formalization of a
folklore observation, and clauses (i) and (iii) are where we place
the content.
...
$Z_d(t;H)$ be its studentization.  The conjecture: for
$H\to\infty$ with $H\le x^{o(1)}$,
\[
...
\]
and the finite-dimensional distributions of $\{Z_d(t;H)\}_{2\mid
d\le2D(x)}$, $D(x)=(\log x)^A$, converge to a centered Gaussian
field with \emph{this} correlation kernel.  The diagonal ($4$-tuple)
correction to $\operatorname{Var}_t(\pi_d)$ below its Poisson
value---the pair analogue of the Montgomery--Soundararajan deficit of
...
\emph{(iii)} [second-order, singular-series dependence] in normal
order over $g\in\mathcal G$,
\[
\log p(g)\;=\;\sqrt g+\tfrac12\log g-\tfrac12\log\SG^{*}(g)+O(1),
\qquad
\SG^{*}(g)=\prod_{\substack{p\mid g\\ p>2}}\frac{p-1}{p-2}:
\]
smooth gaps, whose tuple constant is large, appear \emph{earlier} by
exactly half a logarithm of that constant---the waiting-time
first-passage computation makes the coefficient $-\tfrac12$, not a
free parameter.  (The restriction to $\mathcal G$ repairs a
...
\emph{(v)} [contamination calculus, general form] for \emph{any}
pattern $(n,n+d)$ and modulus $m$, the deterministic drift vector of
the residue-class race is computed by one rule: enumerate the
...
mass, and the uncontaminated symmetric classes lead.  In particular,
...
logarithmic averaging: the class-4 deficit race led on the predicted
side with log-density $0.99$, the class-1 mod-8 race with $0.92$
(against $\tfrac12$ under symmetry, and against arcsine-spread
occupation under the null), while the control differences among
uncontaminated classes stayed within one noise unit of zero in
log-mean.  This is directional support, not a sharp confirmation;
...
\emph{(ii)} [canonical deficit] define, with no reference to any
control model, $\Theta(q)=\bigl(1-\mathbb E_a[U(a,q)]\bigr)\log q$.
The conjecture is that along \emph{prime} moduli $q\to\infty$,
$\Theta(q)$ tends to a positive constant $\Theta$, i.e.
\[
1-\mathbb E_a\,[U]\;=\;\frac{\Theta+o(1)}{\log q}
\qquad(q\text{ prime},\ q\to\infty),
\]
with $\Theta>\theta_{\mathrm{disc}}^{\infty}$, the corresponding
limit for the discrete-hazard control defined below: the primes'
deficit exceeds what discreteness alone produces.
\end{conjecture}
...
specified: for $t$ uniform on $[N,2N]$ and window length $H$
($H\to\infty$, $H\le N^{o(1)}$), let
$Q_A(t;H)=\#\{t<n\le t+H:\ n^2+n+A \text{ prime}\}$.  Two members
share the index window, so with
...
exclusive pairs, and the studentized field converges to a centered
