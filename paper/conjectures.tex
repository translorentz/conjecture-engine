...
law for the mixed polynomial--exponential sequence $n^2+2^n$; and a
...
\emph{Benchmarks} (Conjectures \ref{c1}--\ref{c8}, \ref{c10},
\ref{c12}--\ref{c18}, \ref{c23}--\ref{c25}): statements that are
previously known in essence, or that follow mechanically from
...
\ref{c9}, \ref{c11}, \ref{c19}--\ref{c22}): statements involving a
mechanism, constant, or law we could not find in the literature, stated
with their dependence assumptions explicit.  Within the latter group we
regard Conjecture~\ref{c22}(ii) (the least-prime deficit and ordering
anomaly), Conjecture~\ref{c21} (the twin-race mechanism), and
Conjecture~\ref{c20} (the microscopic variance law) as the paper's
core, with Conjectures~\ref{c9}, \ref{c11} and \ref{c19} carrying
open modeling questions flagged in place.
...
rather than the usual halving.  Among odd shifts $c$, the companions
$c=3$ and $c=9$ are inadmissible at $p=3$, while $c=5,7,11,13,\dots$
survive; an earlier draft wrongly called $+7$ the nearest admissible
shift ($+5$ is admissible, with constant $3.497$), and we record the
correction: the choice $c=7$ is a choice, not a canonical one.
...
prime, partial sums $\e^{\gamma}\log x$.  The sequence is highly
structured and the sample is tiny, so this constant is materially less
secure than a fixed-polynomial Bateman--Horn constant; the statement is
a classical benchmark (Caldwell--Gallot), not a new claim.
...
\[
\tfrac12-s(x)\;=\;\bigl(c_3+o(1)\bigr)\,\frac{\log\log x}{\log x},
\]
where $c_3$ is the explicit constant determined by the
Lemke Oliver--Soundararajan conjectural formula \cite{LOS} specialized
to modulus $3$ (its extraction from their secondary terms is a
computation we have not performed), and the deficit splits
symmetrically between the $(1,1)$ and $(2,2)$ patterns.
\end{conjecture}

This is their conjecture restated as a benchmark, not new content; the
repulsion is real but evanescent---vanishing at the precise rate
$\log\log x/\log x$, unlike Chebyshev's persistent bias.  The
measured proxy $\hat c=0.400,\ 0.375,\ 0.370$ at
$x=10^7,\ 10^9,\ 4\cdot10^9$ (slow drift consistent with the $o(1)$)
is calibration data only---an external review is right that the
constant should be extracted analytically from \cite{LOS}, not fitted
by regression, and no numerical range is part of the statement.  The
symmetry check gives $\#(1,1)/\#(2,2)=0.99989$ at $4\cdot10^9$.
...
constants of C\ref{c2}, C\ref{c4}, C\ref{c5}, C\ref{c7} and
C\ref{c8}.  \emph{Conceptual novelty}---a mechanism, law, or constant
we could not find in the literature: C\ref{c11}'s candidate
local-global product for a mixed polynomial--exponential sequence;
C\ref{c19}'s liminf constant (against the slope-1 laws of Shanks,
Wolf, and Kourbatov--Wolf \cite{KW2020}); C\ref{c20}'s microscopic
variance law (a sharpening of \cite{MS}, with the mesoscopic numerics
of \cite{SL} as precedent); C\ref{c21}'s race mechanism (against the
twin-bias literature \cite{Sahoo,PairBias}); and C\ref{c22}(ii)'s
deficit decomposition.  Among these we consider C\ref{c21} and
C\ref{c22}(ii)---the twin-race mechanism and the least-prime deficit
law with its ordering anomaly---the paper's most genuinely new content,
and Question~\ref{q:theta} its best export.
