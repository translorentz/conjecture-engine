...
\section*{Summary sheet: the twenty-five conjectures, ordered by
importance and novelty}
\addcontentsline{toc}{section}{Summary sheet}

The statements below are given in compressed form and in decreasing
order of a composite editorial judgement of importance and novelty:
mechanism-level novelty ranks above new derived laws inside known
frameworks, which rank above instance-level content; importance
weighs leverage on neighbouring problems, falsifiability with
derived (unfitted) constants, and derivation status.  The ordering
is editorial, not mathematical.  \emph{The numbered statements in
the body are normative}: they carry the ensembles, conditionality
clauses, exceptional cases, and error conventions that are part of
each conjecture and are only abbreviated here.  Notation:
$\M_x(F)=\frac1{\log x}\int_2^xF(t)\,\frac{dt}t$; $\SG$, $C(\cdot)$
singular series and Bateman--Horn constants; $\Li_k(x)=\int_2^x
(\log t)^{-k}dt$.
