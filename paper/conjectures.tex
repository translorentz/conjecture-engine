...
depth that the Finding~\ref{f:c10} failure taught us to demand.  Second, several of the
...
$n\approx10^6,10^7,10^8$.  The \emph{comparative} statement between
the two lanes---which lane leads, by how much, and why---is not this
benchmark but Conjecture~\ref{c25}, where the contamination calculus
of Conjecture~\ref{c21}(v) supplies the mechanism.
...
$1.6835$ against measured $2.7434$ and $1.6726$).
...
C\ref{c10}'s window-rigidity and joint-fluctuation clauses (the
uniqueness core itself is attributed to OEIS A088054 after the
second priority correction of Finding~\ref{f:c10});
...
Several structural programmes raised by the fifth external review are
recorded here as open directions rather than folded into the
twenty-five statements: a boundary-factorization principle
classifying, for representations $F(m)=p+F(j)$, when primality
forces $m-j$ to be bounded (the general home of
Theorem~\ref{t:cube} and Conjecture~\ref{c4}); the admissibility
dynamics of cyclotomic composition words (Conjecture~\ref{c6});
a Waring--Goldbach theory for members of prime constellations
(Conjecture~\ref{c17}); multibase covariance laws for Fermat
quotients (Conjecture~\ref{c23}); and centering-and-scaling
extreme-value laws for record gaps, where Kourbatov's Gumbel-type
framework \cite{Kourbatov} already occupies part of the ground and
any contribution of ours would need separating from it.
