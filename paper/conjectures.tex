...
factors at distinct $p$ are exactly independent as $d$ varies (CRT),
so the moments of $C(d)$ over even $d\le D$ converge, as
$D\to\infty$, to \emph{derived} Euler products; in particular
\[
...
moves the product ($\omega\in\{0,3\}$ by the cubic-residue character
of $-a$; for $p\equiv2\pmod 3$ the cube map is a bijection and the
factor is exactly $1$).  Then:
\emph{(i)} [mean-value one, elementary lemma] $\mathbb E_a[\omega(p)]
=1$ exactly at every $p$, so the derived mean of $C(a)$ is exactly
$1$---the cubic analogue of the Korevaar--te Riele mean-value-one
theorem for linear prime-pair constants (arXiv:0806.1667), here a
...
$\prod_{p\equiv1(3)}f_p(\xi_p)$ with independent cubic-residue
indicators $\xi_p$ (the variance sum converges \emph{without}
normalization, unlike the quadratic family of
Conjecture~\ref{c1}(iii)), with derived standard deviation
$0.2762\ldots$;
...
requires), leadership log-density $0.65$ ($+0.4$ null standard
deviations---directionally consistent, sharpness unavailable at this
height by the calculus's own accounting).  (The Chernick chain
...
four leadership log-densities $(0.48,0.56,0.53,0.73)$ are each within
one corrected-null standard deviation ($0.37$) of $\tfrac12$: the
...
$2\sqrt{\log2/\log x}$ (the OU sign-covariance
$\frac2\pi\arcsin\e^{-u/2}$ integrates to $2\log2$); the classical
...
$0.7$ null standard deviations below $\tfrac12$ (scale
$2\sqrt{\log2/\log x}\approx0.42$ there)---consistent with the
...
leadership $0.21$---$0.7$ null standard deviations below $\tfrac12$
on the corrected clock of clause (ii), the profile of a fair race.  (The quadratic triple $n^2+\{1,3,7\}$ that formerly
...
\emph{(i)} [rank-disjointness, elementary] every prime factor of
$F_p$ ($p\neq5$ prime) has rank of apparition exactly $p$, and every
prime factor of $L_p$ has rank exactly $2p$: the divisor pools of the
two numbers are \emph{disjoint}, so the joint local screening carries
no shared-prime correlation (the structural analogue of
Conjecture~\ref{c10}(i)'s deterministic screening), and
$F_pL_p=F_{2p}$ exhibits each twin as the natural semiprime splitting
of $F_{2p}$;
...
$(\e^{\gamma}\log p/(p\log\phi))^2$ per index---is \emph{refuted} by
the record: it prices the known index $p=148091$ (OEIS A080327,
$F_p$ a probable prime, $L_p$ proven) at ${\sim}3\times10^{-3}$ of
expected mass beyond $10^4$, so the true joint constant exceeds the
naive one by an order of magnitude or the events are enriched beyond
the model; the conjectural content of (ii) is that even the
corrected accounting has convergent sum.
\end{conjecture}
...
divisibility structures are nested.
...
$\sum_{h\le H}\SG(h)(H-h)$ \cite{MS}.  The conjecture: (i) the
reduction holds (given quantitative Hardy--Littlewood for $4$-tuples);
(ii) $-G(H)/\log H$ diverges: the pinned average grows
\emph{strictly faster} than the single-prime secondary term
$\tfrac12(\log H+\gamma+\log2\pi-1)$, i.e.\ pair counts in windows
are more sub-Poisson than prime counts, with the asymptotic form of
$G$ (pure $\log$ with a larger constant, or genuinely $\log^2$)
registered as the open second-order question.
\end{conjecture}

$G$ was computed exactly to $H=3000$ ($4$-tuple constants by Euler
products to $2\times10^4$): $G(3000)=-20.9$, with local logarithmic
slope increasing from $3.4$ to $3.7$ across the range---already
seven times the single-prime slope $\tfrac12$, establishing (ii)
over the computable range and leaving the limiting form genuinely
...
For $F\in\mathbb Z[x]$, $(m-j)\mid F(m)-F(j)$ (textbook), so
$F(m)-F(j)$ prime forces $|m-j|=1$ apart from the finitely many pairs
where the cofactor is $\pm1$ (an effectively boundable exceptional
set; for the family below it is provably \emph{empty}): every
representation problem $F(m)=p+F(j)$ collapses to the boundary
polynomial $D_F(m)=F(m)-F(m-1)$.  The content is the classification
...
$k$:
\emph{(i)} [class assignment, elementary genus theory] contamination
...
($x^2+2y^2$ represents no such $n$---classical genus theory of
discriminant $-8$, so the null classes are provable, not
conjectured);
...
\mathrm{prime}}2\log q\bigr]/\lambda(n)$ with $\lambda(n)$ the lane
mean of $\log p$; on the null classes the lane difference has zero
drift.
\end{conjecture}
...
$D_{\mathrm{sys}}=+0.30$; null strata pooled $-0.22\pm0.33$,
...
\emph{(i)} [limit law] $U\Rightarrow\mathrm{Exp}(1)$ for $n$
log-sampled from the even integers;
\emph{(ii)} [canonical ordering deficit] the model-free deficit
$\Theta_G(n)=(1-\mathbb E[U])\log n$ tends, in dyadic average, to a
positive constant $\Theta_G$---the Goldbach sibling of the
...
arrival comes \emph{early} and $\mathbb E[U]<1$.
\end{conjecture}
...
displacement itself, as (ii) requires.  (The $(3,3)$-lane refined
...
\]
uniformly for $H\le x^{o(1)}$; when $H/\log^2x\to\lambda$ the
window counts converge jointly to correlated Poisson laws with these
covariances, and when $H/\log^2x\to\infty$ every fixed finite
collection $\{Z_{d_1},\dots,Z_{d_r}\}$ is asymptotically jointly
normal.  Since $\rho\asymp1/\log x\to0$, the limiting coordinates
are independent: the kernel is the \emph{first-order correction to
independence}, a quantitative finite-$x$ law (test below), not a
nondegenerate limiting kernel.  The diagonal ($4$-tuple) correction
to $\operatorname{Var}_t(\pi_d)$ below its Poisson value---the pair
...
\int_5^{n-5}\frac{dt}{\log^2t\,\log^2(n-t)}\;\bigl(1+O(1/\log
n)\bigr),
\]
...
\emph{(i)} [serial repulsion] the step autocorrelations are negative
at small lags---the race analogue of the Lemke Oliver--Soundararajan
consecutive-pattern repulsion \cite{LOS}, which supplies the
mechanism and, in principle, the constants---with
$\rho_1\approx-0.037$, $\rho_2\approx-0.009$ measured universally
across twin and cousin races mod $5$ and mod $8$ at $10^9$ (four
races, agreeing to within $0.004$, each ${\sim}50$ standard errors
from zero);
\emph{(ii)} [long memory] short lags do not exhaust the deficit: the
running maxima $M(x)=\max_{t\le x}|D(t)|/\sqrt{\text{count}}$ of the
four races land at quantiles $0.02$--$0.17$ of the simulated
independent-step null (median $2.34$ at this span), and still only
$0.03$--$0.23$ after correcting for $\rho_{1,2}$: the negative
correlation persists across lags, and the cumulative diffusivity
$\sigma^2(x)=1+2\sum_k\rho_k(x)$ decreases with observation scale;
...
Cram\'er--Granville maximal-gap envelope---this clause is the
inversion of Granville's $\limsup G(X)/\log^2X=2\e^{-\gamma}$ into
first-occurrence coordinates, and its value is registered
\emph{as} that dual (against the slope-1 laws), not claimed as an
...
$G\to\infty$,
\[
\log p(g)\;=\;\sqrt g+\tfrac12\log g-\tfrac12\log\SG^{*}(g)+O_P(1),
\qquad
\SG^{*}(g)=\prod_{\substack{p\mid g\\ p>2}}\frac{p-1}{p-2},
\]
the $O_P(1)$ term conjecturally following a first-passage
(Gumbel-type) law: smooth gaps, whose tuple constant is large, appear
\emph{earlier} by exactly half a logarithm of that constant---the
waiting-time first-passage computation makes the coefficient
$-\tfrac12$, not a free parameter.  (The restriction to $\mathcal G$ repairs a
...
deterministic drift vector is computed by one rule: enumerate the
prime-square orientations $(q^2-d,\,q^2)$ and $(q^2,\,q^2+d)$
surviving all local obstructions; the classes of their surviving
members mod $m$ receive $\Lambda$-weighted contamination; the
unweighted pattern counts are depressed there by exactly the removed
mass, and the uncontaminated symmetric classes lead.  (Races with
unequal primary constants, or with several interfering orientations,
are outside the calculus until those terms are separated---a
restriction the sixth external review is right to demand.)  In
particular,
...
logarithmic averaging, now calibrated against the corrected null of
Conjecture~\ref{c8}(ii) (log-occupation $\tfrac12$ with Gaussian
spread $2\sqrt{\log2/\log x}\approx0.37$ at $10^9$): the class-4
deficit race led on the predicted side with log-density $0.99$
($+1.3$ null standard deviations), the class-1 mod-8 race with
$0.92$ ($+1.1$), both signs agreeing with the prediction, while the
control differences among uncontaminated classes stayed within one
noise unit of zero in log-mean.  This is directional support, not a
sharp confirmation;
...
\;\longrightarrow\;\Theta\;>\;\theta_{\mathrm{disc}}^{\infty}
\qquad(Q\to\infty),
\]
with the per-modulus spread $\operatorname{sd}(\Theta(q))$ over the
dyadic block tending to $0$ at a rate left open: the primes'
average deficit exceeds what discreteness alone produces.
\end{conjecture}
...
$W(x)=\#\{\text{Wieferich } p\le x\}$ satisfies the exact
independent-Bernoulli fluctuation laws: the central limit theorem
$(W(x)-\log\log x)/\sqrt{\log\log x}\Rightarrow N(0,1)$ along with
...
\]
(the law of the iterated logarithm at variance $V(x)\sim\log\log x$;
an earlier draft's $O_\varepsilon(V^{1/2+\varepsilon})$ was, as the
sixth review noted, weaker than the model's exact prediction, and
this paper's calibration principle cuts both ways).
\end{conjecture}
...
exclusive pairs; for $H/\log N\to\lambda$ the counts are jointly
...
jointly normal, the correlations of size $\asymp1/\log N$ making the
kernel a first-order correction to independence, exactly as at
...
diagonal law is the family's remaining open component---and, as the
sixth review stresses, the more important one; the numerical
cancellation of the cross-member kernel is a finite computation over
$A\le99$, which localizes the deficit in the diagonal without
proving the averaged kernel vanishes---an analytic classification of
the sign of $C(A,A')-C(A)C(A')$ remains open.
