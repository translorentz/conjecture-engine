...
ladder for $n=p+j^k$ (total algebraic obstruction for even $k\ge4$, a
Bateman--Horn lane for odd $k$), the finiteness of primorial twins (the
convergent-side Borel--Cantelli exemplar), and a Goldbach lane race in
which prime-square contamination pushes every bias term into the
$(1,1)$ lane.
\end{abstract}
...
\begin{conjecture}[The power-obstruction ladder; apparently new as a
family]\label{c4}
For $k\ge2$ and $m\ge2$, write $D_k(m)=m^k-(m-1)^k$.  Then, with
representability meaning $m^k=p+j^k$ for some prime $p$ and $j\ge1$:
\emph{(i)} [theorem] for even $k\ge4$, NO $k$-th power is representable
($m^k-j^k$ factors through $m^{k/2}\pm j^{k/2}$, and at $j=m-1$ both
factors exceed $1$);
\emph{(ii)} [elementary] for $k=2$ and odd $k\ge3$, $m^k$ is
representable if and only if $D_k(m)$ is prime;
\emph{(iii)} [conjecture] for each such $k$ the representable lane
follows Bateman--Horn:
$\#\{m\le M:\ D_k(m) \text{ prime}\}\sim C_k\int_2^M dt/\log D_k(t)$,
where $C_k$ is the singular series of the irreducible polynomial $D_k$.
\end{conjecture}

This family grew out of the audit refutation behind
Conjecture~\ref{c13} (the case $k=3$): the same factorization that
destroyed the naive cube conjecture organizes, for every $k$, into an
even/odd dichotomy---total algebraic obstruction for even $k\ge4$, a
one-parameter Bateman--Horn lane for $k=2$ and odd $k$.  We could find
no prior statement of the ladder.  It replaces, at the suggestion of an
external review, a bibliographically-novel-at-most quintuplet
...
singular series simply absorbs the resulting confinement to
$n\equiv0\bmod5$); we stop at the triple studied here, and claim no
canonical status for that choice.
...
$\approx7\times10^{-3}$ (computed tail), so the three-element list
should be complete outright.
...
determination of the functional form.
...
$1\le A\le(\log N)^B$, with jointly Poisson-size residuals.
...
\log q\,\log(n-q^2),
\qquad
\bar\ell(n)=\text{mean of } \log p\log(n-p) \text{ over the }(3,3)
\text{ lane},
\]
the mean of $D-D_{\mathrm{sys}}$ over $n\equiv2\ (4)$, $n\le x$, is
$o$ of the mean of $D_{\mathrm{sys}}$; per-$n$, $D$ is
$D_{\mathrm{sys}}$ plus noise of the square-root scale, so the sign of
$D$ favours the $(3,3)$ lane only weakly at any fixed $n$.
\end{conjecture}
