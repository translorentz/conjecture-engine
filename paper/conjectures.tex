...
(conditionally on the probable-prime status of both $F_{148091}$ and
$L_{148091}$).
...
are presented in ascending numerical order, in three sections: the
...
slot's stated programme.  Note that $\SG(n)$ alone does not encode the
...
is not a statement about machine precision).  Entanglement is thus
...
uniformly for all $d,d'\le(\log x)^{A}$---the per-shift centered average
...
height.  The model layer is declared rather than assumed silently:
...
stationarity is assumed at all.  The Green--Kubo sum requires more than the fixed-lag law
...
it earns that role by \emph{failing} the naive version: our
scan to $p\le10^4$ found exactly the joint indices
$\{5,7,11,13,17,47\}$ with expected further mass $1.6\times10^{-3}$
---and the catalogued index $148091$, at which $F_p$ and $L_p$ are
both probable primes, exists anyway.  We therefore
...
clause (ii) is attributed there and the slot's claimed content is
...
repository artifacts---verifier scripts and results files---carry a
