...
into Conjecture~\ref{c9} as its calibration clause.  Second, several of the
...
$\M_x((D-T)/\sqrt\pi)\to0$, a form the ninth external review showed
to be vacuous for the drift coefficient (both $T/\sqrt\pi$ and its
multiples have logarithmic mean $\to0$).  Convergence of $\M_x$ at
...
needs more than almost-sure convergence (the eighth review's
caution), and the needed ingredient is on hand: the convergent
...
configuration $(3,5,9)$ occurs---the ninth review's catch, one term,
harmless to every asymptotic)---leaving the single
...
in the drift-scale form of Conjecture~\ref{c21}(i) (the
$\M_x((D-T_3)/\sqrt{\pi_3})$ form of an earlier draft was vacuous
for the coefficient, as at the pair races):
\[
\M_x\!\left(
...
$r$ exists precisely because $k$ is composite; $r=1$ would allow the
useless factor $m-j=1$ at $j=m-1$, a gap in an earlier phrasing
noted by the eighth review), then for every
...
statement uncovers, and both clauses were found by the engine's own
...
$3\mid\Phi_3(u)$ always, while $p\equiv1\pmod 3$ forces $3\mid u$; the
engine's admissibility check rejected the naive chain at design time.
...
Conjecture~\ref{c21}(i) (the earlier $\M_x$-over-$\sqrt\pi$ phrasing
was coefficient-blind, per the ninth review), is:
...
Conjecture~\ref{c21}(i) for this pair (the ninth review observed
that the earlier normalization $\M_x(Y)\to0$ with
$Y=D/\sqrt{\pi^{\mathrm q}}$ was too weak to exclude a drift at the
contamination scale, hence too weak to make this race a control;
the restated form excludes exactly that);
\emph{(ii)} [occupation law, corrected clock, stated as a
conditional consequence] the arithmetic content of this clause is an
\emph{invariance principle}, for the process now defined precisely
(the ninth review asked for the event indexing): let
...
Brownian motion on $\tau\in[0,1]$.  Everything after that is probability, not
number theory (the eighth review's demanded separation, which we
adopt): granting the invariance principle, $D(t)\approx B(N(t))$
...
calibration members of repository slots $6$, $7$ and $8$
(Conjectures~\ref{c6}, \ref{c7} and~\ref{c8}; the
...
formerly occupied repository slot $4$ (Conjecture~\ref{c4})
retains its audit-record calibration:
...
that PRP being prime---the ninth review's caution, which we adopt:
no unconditional refutation is claimed from an uncertified number)
at
that much expected mass beyond $10^4$.  A single surprising event is
not a strict refutation, and---a further caution from the same
review---it does not by itself estimate the \emph{size} of the
correction: an underestimated constant, genuine cross-dependence,
...
control alone cannot preclude higher-order dependence, as the ninth
review correctly insisted---the higher-cumulant condition
...
asserted separately (the eighth review's demanded separation):
...
expressly not counted among this paper's conjectural assertions (the
ninth review's point: a question is not a conjecture), on
...
the order of the claimed error and cannot be dropped, an omission in
an earlier draft caught by the ninth external review (it is the same
diagonal that, in the single-prime case, merges into
Montgomery--Soundararajan's constant $\gamma+\log2\pi-1$).  With
that term restored, the entire deficit left open at
...
fails (an omission caught by the seventh review), and for negative
leading coefficient the divided difference tends to $-\infty$ on the
cone, so the ``exceeds $1$'' clause is false as stated (caught by
the eighth review; one may equivalently keep general sign and read
the claim through $|F(m)-F(j)|$).  Every representation problem $F(m)=p+F(j)$ in this range
...
Conjecture~\ref{c25}, which the ninth review rightly demanded as
the template---an earlier draft's ``$D=D_{\mathrm{sys}}+$noise in
the log-sampled mean'' specified neither the measure, the sign, nor
the limit relation]  Fix the ensemble: for a scale $X$, let
...
factor $2$ belongs to Conjecture~\ref{c25}, not here)---where
...
depth, prompted by the eighth review: an \emph{asymptotic} for the
...
average of Conjecture~\ref{c12}---where, per the eighth review's
caution, degenerate offset relations are handled inside the kernels,
not ignored: whenever the four-point set collapses (repeated
...
dominance range is stated as an explicit function class rather than
the imprecise ``narrower than $x^{o(1)}$'' of an earlier draft (the
ninth review's objection): for $H=\lambda(\log x)^{\alpha}$, any
...
the evidence.  Two precision notes per the ninth review: the
...
registered identity-hunting programme (the eighth review's
suggestion, which we adopt).  Verified on $150$ log-sampled $n\le10^8$: the profile
...
now defined at event index (the ninth review's demand): enumerate
...
of (i), and the eighth review is right to insist on the gap: the
...
$K(x)\to\infty$, \emph{and}---the ninth review's further catch: the
tail bound alone leaves the growing head sum uncontrolled---the
...
$\Lambda_g(y)=W$ gives $E_g=y-y_0=\tfrac12\log W+o(1)$---an earlier
draft wrote the law at scale $1$, overlooking that the slope of the
log-intensity at $y_0$ is $2$, not $1$ (both the $y$-term and the
$g/y$-term contribute); the eighth external review caught this, and
the same slope-$2$ expansion is what produces the $\tfrac12$
coefficients of the centering.  (Gumbel-type modeling of gap
...
not the extreme-value framework.  One selection caveat, per the
ninth review: sampling uniformly over the \emph{realized} gaps in
...
is actually identified.  An earlier draft wrote
$\M_x((D_1-T)/\sqrt{\pi_t})\to0$; the ninth external review observed
that this is \emph{vacuous} for the coefficient---since
...
does not express ``exactly the removed mass.''  The repaired form
normalizes at the \emph{drift} scale, where the claimed limit is a
nonzero constant:
\[
...
census, is the mathematical content).  The operating scale should be
...
the residue is called an anomaly, and the eighth external review is
right to demand it: for prime $q$, the primes $p<q$ occupy
...
$1/\log q$ decay that the flat measured values do not show.  (The
review suggested the effect sits at the full deficit scale because
the \emph{affected-class fraction} is $\pi(q)/\varphi\asymp1/\log q$;
but the deficit is the affected fraction times the collision
...
$(W(x)-u)/\sqrt u$ converges to $N(0,1)$ as $X\to\infty$.  Two
successive drafts of this clause were wrong, and the corrections are
instructive enough to record.  A block-sampled version on $[X,X^A]$
fails because only $\log A=O(1)$ new events enter the block (eighth
review).  A \emph{uniform}-in-$u$ version on $[1,L]$ fails too, as
the ninth review showed by Brownian scaling: with
...
open component.  (An earlier draft divided this sum by a further
$\log^2N$; the ninth review caught the error---the joint event here
is \emph{two single prime values} at distinct indices, probability
$\asymp1/4\log^2N$, unlike Conjecture~\ref{c16}'s pair-of-pairs
events at $1/\log^4x$.  The correction reverses an inference: at
the right order the off-index sum, which grows like a pinned
...
observed profile deficit to the diagonal is hereby downgraded from
an inference to an open question.)  For $H/\log N\to\lambda$ the counts are jointly
...
cross-member kernel is null.  With the off-index sum now at its
correct order (larger, not smaller, than the same-index term), this
cancellation no longer licenses the conclusion that the members are
...
\ref{c18}, \ref{c19}, and~\ref{c20}, and the quintuplet count of
Conjecture~\ref{c4}); the
...
repository artifacts---verifier scripts, results files, and audit
documents---retain the original thematic slot numbering under which
they were produced.  The map from conjecture number to repository
...
of repository slot $10$ (Conjecture~\ref{c10}), Lillie
\cite{Lillie} (Finding~\ref{f:c10}); the
primorial benchmark before that and the Lemke Oliver--Soundararajan
restatement of repository slot $25$ (Conjecture~\ref{c25}),
\cite{CaldwellGallot} and \cite{LOS};
the Sophie Germain statement of repository slot $6$
(Conjecture~\ref{c6}) and the quadratic triple of repository slot
$8$ (Conjecture~\ref{c8}).  Framework attributions for claimed clauses
