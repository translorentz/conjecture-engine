...
associated $q$-level stabilizer graph state; label zero means that the edge is
absent.  We write $\operatorname{rw}$
...
~\cite{JavelleCattaneoPerdrix12,JavelleMhallaPerdrix15}.  The mean has the
...
$A\triangle B$ create structured dependencies~\cite{HaoEtAl20}.
...
~\cite{DeJongVandreBurchardt24}; structural descriptions through minimal local
sets give another nearby boundary~\cite{ClaudetPerdrix24}.  Those results are
...
~\cite{Csikvari10}, but no theorem located in the audit gives the all-threshold
